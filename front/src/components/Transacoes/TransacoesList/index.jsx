...
export const TransacoesList = () => {
    ...
    useEffect(() => {
        if(ano === 'todos') {
            ...
            if(tipo === 'Todas'){
                ...
                (transacao => new Date(transacao.data)
                .getFullYear() === Number(ano));
                setTransacoesTable(todas);
            }
            if(tipo === 'Receitas'){
                const receitas = transacoes.filter
                (transacao => transacao.tipo === 'Receita' && 
                new Date(transacao.data).getFullYear() === Number(ano));
                setTransacoesTable(receitas);
            }
            if(tipo === 'Despesas'){
                const despesas = transacoes.filter
    ...
    return (
       ...
       >
            <div onClick={() => setTipo('Todas')}>
                Todas Transações
            </div>
            <div onClick={() => setTipo('Receitas')}>
                Receitas
            </div>
            <div onClick={() => setTipo('Despesas')}>
                Despesas
            </div>
       </div>
                ...
                <S.TableBody>
                    {transacoesTable.map((transacao) => (
                        ...
                        >
                            ...
                            <S.TableCell align="right">
                                {compareAsc(new Date(), new Date(transacao.data)) === 1 ? 'Realizada' : 'Planejada'}
                            </S.TableCell>
                            <S.TableCell align="right">
