...
export const CreateTransacoes = ({openModal, closeModal }) => {
   ...
   const onSubmit =  async (e) => {
    ...
    try {
       const token = localStorage.getItem('token');
       await axios.post('http://localhost:8080/transacoes', { descricao, valor: valor * 100, data: dataTransacao, tipo, category: categoria}, {
        headers: {
            'Authorization': ` Bearer ${token}`
        }
        })
   ...
   const handleClose = (_, reason) => {
    ...
    <S.Snackbar open={notification.open} autoHideDuration={3000} onClose={handleClose}>
        ...
        <Dialog open={open} onClose={handleCloseModal}>
            <DialogTitle>
            <div style={{ textAlign: 'center', marginBottom: '32px' }}>Nova Transação</div>
            </DialogTitle>
            <DialogContent>
            ...
            <input
              ...
              value={dataTransacao}
              onChange={onChangeValue}
              style={{ padding: '10px', marginTop: '5px', marginBottom: '20px', borderRadius: '5px', border: '1px solid #ccc', width: '100%' }}
            />            
            </S.Form>
            </DialogContent>
            <DialogActions>
